  ...
  return `
    <header class="topbar">
      <div class="topbar-inner">
        ...
        <span class="search-hint">${integer.format(data.config.counts.people)} персон</span><a class="about-link" href="#/methodology">Как формировалась карта</a>
      </div>
    </header>`;
}
...
function footer() {
  return `<footer class="footer compact-footer"><div class="footer-inner"><div class="footer-columns">
    <div><h2>Ресурсы для развития НКО</h2><nav aria-label="Ресурсы для развития НКО">
      <a href="https://kocheva.github.io/-/" target="_blank" rel="noopener noreferrer">Навигатор образовательных курсов ↗</a>
      <a href="https://navigator-nko.ru/" target="_blank" rel="noopener noreferrer">Навигатор поддержки НКО ↗</a>
      <a href="#/methodology">Методология и источники</a>
    </nav></div>
    <div class="footer-project"><p>Создано в рамках проекта ${publicSourceLink(PROJECT_URL, "«Техникум местного эксперта: развитие кадрового потенциала для устойчивых сообществ Челябинской области»")} при поддержке ${publicSourceLink(FOUNDATION_URL, "Фонда «Центр поддержки гражданских инициатив и развития некоммерческого сектора экономики Челябинской области»")}</p>
    ${publicSourceLink("https://grany-center.org/", "Центр ГРАНИ ↗")}</div>
    </div><p class="footer-note">Данные собраны из открытых источников. Неизвестные суммы не включены в расчёт финансирования.</p></div></footer>`;
}

function methodologyPage() {
  return `${header()}<main id="main" class="page-shell methodology-page"><a class="back-link" href="#/">← Вернуться к карте</a><h1>Методология и источники</h1>
  ...
  <section class="detail-section">${sourcesContent()}</section></main>${footer()}`;
}
